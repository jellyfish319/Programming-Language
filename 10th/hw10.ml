...
let rec interp (exp : Ast.expr) (ls : Store.t) : Store.value =
  ...
  | Ast.Id x -> 
    let v = Store.find x ls in
    (match v with
    | Store.FreezedV (e, sigma) -> interp e sigma  (*v의 값이 Freezed이면 녹임*)
    | _ -> v)
  ...
  (*lazy evaluation이 기본 정책*)
  | Ast.App (e1, e2) -> 
    (match interp e1 ls with
    | Store.ClosureV (par, e3, sigma) ->
      let arg = Store.FreezedV (e2, ls) in (*e2와 당시의 추상 메모리를 얼림*)
      interp e3 (Store.add par arg sigma) (*얼린 상태로 저장*)
    | _ -> failwith (Format.asprintf "Not a function: %a" Ast.pp e1))
